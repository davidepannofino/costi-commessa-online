...
// (es. "Costi Commessa <no-reply@tuodominio.it>") senza toccare altro codice.
const MITTENTE = process.env.EMAIL_MITTENTE || "Costi Commessa <onboarding@resend.dev>";

export async function inviaEmailResetPassword(email, link) {
  await resend.emails.send({
    from: MITTENTE,
    to: email,
    subject: "Reimposta la tua password — Costi Commessa",
    text:
      "Hai richiesto di reimpostare la password del tuo account Costi Commessa.\n\n" +
    ...
    html: `
      <div style="font-family:Arial,sans-serif;color:#22262E;max-width:480px;margin:0 auto;">
        <p style="font-size:16px;">Hai richiesto di reimpostare la password del tuo account <strong>Costi Commessa</strong>.</p>
